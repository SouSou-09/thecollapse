// arc-sw-core.js is imported once in sw.js — do NOT re-import here to avoid SW conflict
...
class UVServiceWorker extends EventEmitter {   
    ...
    async fetch({ request }) {
        ...
        try {
            ...
            } else if (requestCtx.headers.origin) {
                // No valid referrer: rewrite proxy origin → target origin to prevent CORS failures
                requestCtx.headers.origin = ultraviolet.meta.url.origin;
            };
            ...
            if (responseCtx.headers['set-cookie']) {
                // Await cookie persistence so deletion only happens after a successful save.
                // The original fire-and-forget Promise caused cookies to be lost if setCookies failed.
            ...
            if (responseCtx.body) {
                const contentType = (responseCtx.headers['content-type'] || '').split(';')[0].trim();

                // DRM / EME licence responses are binary (protobuf / octet-stream).
                // Never attempt to rewrite them as text — doing so corrupts the payload.
                const isBinaryLicense = (
                    contentType === 'application/octet-stream' ||
                    contentType === 'application/x-www-form-urlencoded+protobuf' ||
                    contentType === 'application/vnd.ms-playready.initiator+xml' ||
                    contentType === 'video/mp4' ||
                    contentType === 'audio/mp4'
                );

                if (!isBinaryLicense) switch(request.destination) {
                    case 'script':
                    case 'worker':
            ...
            // Re-inject COEP+COOP for document/iframe so SharedArrayBuffer (Unity WebGL etc.) works.
            // These headers were intentionally kept out of the csp-strip list above.
            if (request.destination === 'document' || request.destination === 'iframe') {
                ...
                responseCtx.headers['cross-origin-opener-policy'] =
                    responseCtx.headers['cross-origin-opener-policy'] || 'same-origin';
                responseCtx.headers['cross-origin-resource-policy'] =
                    responseCtx.headers['cross-origin-resource-policy'] || 'cross-origin';
            };

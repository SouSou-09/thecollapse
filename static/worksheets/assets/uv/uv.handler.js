...
async function __uvHook(window, config = {}, bare = '/bare/') {
    ...
    // ââ EME / Widevine DRM Proxy âââââââââââââââââââââââââââââââââââââââââââââââ
    // Routes JS-layer license requests (fetch / XHR) through the UV bare server
    // so the Origin seen by the licence server is the content origin, not the
    // proxy origin.
    //
    // What this fixes (L3 software Widevine, PlayReady, ClearKey):
    //   â¢ Licence fetch/XHR is automatically intercepted and re-routed.
    //   â¢ Origin & Referer headers are rewritten by uv.sw.js (already patched).
    //   â¢ Binary (ArrayBuffer / Blob) request bodies are preserved intact.
    //
    // What this CANNOT fix:
    //   â¢ L1 hardware-backed Widevine: CDM talks directly to Widevine servers
    //     at the OS/TEE level, completely bypassing JS and the Service Worker.
    //   â¢ Device-attestation tokens bound to the original content domain.
    //
    // Public API exposed on __uv.drm:
    //   __uv.drm.setLicenseUrl(session, url)  â register licence URL explicitly
    //   __uv.drm.proxyLicense(session, msg)    â manually trigger proxy request
    // ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
    if (!worker && window.MediaKeys && window.MediaKeySession) {
        // Per-session state: licenceUrl, pendingChallenge
        const _sessionMeta = new WeakMap();

        // ââ 1. Wrap MediaKeys.createSession ââââââââââââââââââââââââââââââââââ
        const _nativeCreateSession = MediaKeys.prototype.createSession;
        MediaKeys.prototype.createSession = function uvCreateSession(sessionType) {
            const session = _nativeCreateSession.call(this, sessionType || 'temporary');
            _sessionMeta.set(session, { licenseUrl: null, pending: false });

            // Listen for licence challenges on this session.
            session.addEventListener('message', function uvOnMessage(event) {
                if (event.messageType !== 'license-request' &&
                    event.messageType !== 'license-renewal') return;

                const meta = _sessionMeta.get(session);
                if (!meta) return;

                // Mark that the NEXT POST fetch/XHR is likely the licence request.
                meta.pending = true;
                meta.challenge = event.message;
                meta.session = session;

                // Auto-clear the pending flag after 10 s to avoid false positives.
                setTimeout(() => { if (meta) meta.pending = false; }, 10000);
            });

            return session;
        };

        // ââ 2. Wrap generateRequest to track initDataType âââââââââââââââââ
        const _nativeGenerateRequest = MediaKeySession.prototype.generateRequest;
        MediaKeySession.prototype.generateRequest = function uvGenerateRequest(initDataType, initData) {
            const meta = _sessionMeta.get(this);
            if (meta) meta.initDataType = initDataType;
            return _nativeGenerateRequest.call(this, initDataType, initData);
        };

        // ââ 3. Intercept fetch to auto-detect & proxy licence POST requests ââ
        // UV's client.fetch hook already rewrites the URL; here we additionally
        // mark binary POSTs that coincide with a pending challenge so we can
        // call session.update() with the response automatically.
        const _nativeFetch = window.fetch;
        window.fetch = async function uvDrmFetch(input, init) {
            // Find any session that is waiting for a licence response.
            // (There is typically only one active session at a time.)
            let activeMeta = null;
            if (init && (init.method || '').toUpperCase() === 'POST') {
                // Walk all tracked sessions to find one with a pending challenge.
                // WeakMap doesn't support iteration, so we use a Set side-channel.
                for (const meta of _uvDrmSessions) {
                    if (meta.pending) { activeMeta = meta; break; }
                }
            }

            const response = await _nativeFetch.call(window, input, init);

            if (activeMeta && activeMeta.pending) {
                activeMeta.pending = false;
                // Clone the response so we can read the body AND return it.
                const clone = response.clone();
                clone.arrayBuffer().then(license => {
                    if (activeMeta.session && license.byteLength > 0) {
                        activeMeta.session.update(license).catch(e => {
                            console.warn('[UV-DRM] session.update() failed:', e);
                        });
                    }
                }).catch(() => {});
            }
        ...
        // Side-channel Set so fetch wrapper can iterate active sessions.
        const _uvDrmSessions = new Set();
        const _origCreateSession2 = MediaKeys.prototype.createSession;
        MediaKeys.prototype.createSession = function uvCreateSession2(sessionType) {
            const session = _origCreateSession2.call(this, sessionType);
            const meta = _sessionMeta.get(session);
            if (meta) _uvDrmSessions.add(meta);
            session.closed.then(() => { if (meta) _uvDrmSessions.delete(meta); }).catch(() => {});
            return session;
        };

        // ââ 4. Intercept XHR for licence requests (binary POST) âââââââââââ
        const _nativeXHRSend = XMLHttpRequest.prototype.send;
        const _nativeXHROpen = XMLHttpRequest.prototype.open;
        XMLHttpRequest.prototype.open = function uvDrmXHROpen(method, url, ...rest) {
            this.__uvDrmMethod = method;
            return _nativeXHROpen.call(this, method, url, ...rest);
        };
        XMLHttpRequest.prototype.send = function uvDrmXHRSend(body) {
            if ((this.__uvDrmMethod || '').toUpperCase() === 'POST' &&
                (body instanceof ArrayBuffer || body instanceof Uint8Array || body instanceof Blob)) {
                // Check for a pending session challenge.
                for (const meta of _uvDrmSessions) {
                    if (meta.pending) {
                        const session = meta.session;
                        meta.pending = false;
                        this.addEventListener('load', function () {
                            if (this.status >= 200 && this.status < 300 && session) {
                                const license = this.response;
                                const buf = license instanceof ArrayBuffer
                                    ? license
                                    : new TextEncoder().encode(license).buffer;
                                session.update(buf).catch(e => {
                                    console.warn('[UV-DRM] XHR session.update() failed:', e);
                                });
                            }
                        });
        ...
        // ââ 5. Public __uv.drm API âââââââââââââââââââââââââââââââââââââââ
        __uv.drm = {
            /**
             * Explicitly register the licence server URL for a session.
             * Call this before or immediately after session.generateRequest().
             * @param {MediaKeySession} session
             * @param {string} url  â full HTTPS URL of the licence server
             */
            setLicenseUrl(session, url) {
                const meta = _sessionMeta.get(session);
                if (meta) meta.licenseUrl = url;
            },

            /**
             * Manually proxy a Widevine challenge through the UV bare server.
             * Returns the raw licence as an ArrayBuffer.
             * @param {MediaKeySession} session
             * @param {BufferSource} challenge  â from MediaKeyMessageEvent.message
             * @param {string} [licenseUrl]     â override; falls back to setLicenseUrl value
             */
            async proxyLicense(session, challenge, licenseUrl) {
                const meta = _sessionMeta.get(session);
                const url = licenseUrl || (meta && meta.licenseUrl);
                if (!url) throw new Error('[UV-DRM] No licence URL registered. Call __uv.drm.setLicenseUrl(session, url) first.');

                // __uv.rewriteUrl routes the request through the UV SW â bare server.
                const proxiedUrl = __uv.rewriteUrl(url);
                const resp = await _nativeFetch.call(window, proxiedUrl, {
                    method: 'POST',
                    body: challenge,
                    headers: { 'Content-Type': 'application/octet-stream' },
                });
                if (!resp.ok) throw new Error(`[UV-DRM] Licence server responded ${resp.status} ${resp.statusText}`);
                return resp.arrayBuffer();
            },
        };
    }
    // ââ End EME / Widevine DRM Proxy âââââââââââââââââââââââââââââââââââââââââââ
};